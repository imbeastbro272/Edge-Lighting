/*
 * LED Brightness Controller - KNN (TIME-LOCAL) LEARNING + UTILITY-BASED AGENT
 *
 * This sketch is the KNN learning controller (identical prediction pipeline:
 * frozen Decision Tree + time-local KNN residual correction) with an ADDITIVE,
 * ADVISORY classical Utility-Based Agent layered on top.
 *
 *   - The Decision Tree (tree_rules.h) + KNN logic is UNCHANGED. The agent never
 *     modifies predictions; it only READS the ML brightness (as a comfort anchor)
 *     plus the sensor/clock inputs.
 *   - The agent is ADVISORY BY DEFAULT (util_apply = false). The LED behaves
 *     EXACTLY like the plain KNN controller until you type "utilon".
 *   - Agent config (weights + night-motion floor) lives in RAM only. It does NOT
 *     touch the EEPROM layout and resets to defaults on reboot.
 *
 * UTILITY AGENT (see utility_agent.h):
 ...
 *   Decision: argmax over b in {0,5,...,100} s.t. the constraint.
 *
 * EXTRA COMMANDS (in addition to all KNN commands):
 *   utility            - print agent breakdown (inputs, weights, floor status,
 *                        per-term contributions, recommended vs ML anchor)
 *   utilon             - enable the agent (it drives the LED)
 *   utiloff            - advisory only (LED uses the ML output) [default]
 *   setutil C E M [F]  - set weights comfort/energy/smooth (>=0) and optional
 *                        night-motion floor% F (0..100)
 *
 * ----- everything below the agent additions is the KNN controller as-is -----
 *
 * APPROACH:
 *   - The pre-trained Decision Tree (2000 samples) stays FROZEN as the base
 *     predictor (tree_rules.h). It is never modified.
...
#include "utility_agent.h"   // additive: classical utility-based agent
...
// ============================================
// UTILITY-BASED AGENT (additive, advisory by default)
//   - util_cfg lives in RAM ONLY (never written to EEPROM).
//   - util_apply == false  => LED uses the ML output (default, unchanged behavior).
//   - util_apply == true   => LED uses the agent's recommended brightness.
//   - agent_prev_brightness feeds the Smooth() term on the next tick.
// ============================================

UtilityConfig util_cfg = { 1.0f, 0.3f, 0.2f, 40.0f };  // wC, wE, wS, night floor%
bool          util_apply = false;
float         agent_prev_brightness = 0.0f;
...
void  cmdUtility();   // additive: utility agent breakdown
...
void setup() {
  ...
  Serial.println(F("==================================="));
  Serial.println(F("LED Controller v4.2 - KNN LEARNING + UTILITY AGENT"));
  Serial.println(F("==================================="));
  ...
  Serial.println(F("Agent      : Utility-based (ADVISORY by default; 'utilon' to enable)"));
...
void loop() {
  ...
  if (!manual_input_mode && current_time - last_update >= UPDATE_INTERVAL) {
    ...
    // --- ADDITIVE: utility agent evaluation (READ-ONLY; uses ml_brightness as
    //     the comfort anchor). Does not alter ml_brightness or the pipeline. ---
    bool agent_is_night = (getTimePeriod(hour) == 4);
    UtilityResult util_eval = utilityEvaluate(
      ml_brightness, agent_prev_brightness,
      agent_is_night, motion_detected != 0, util_cfg);

    manual_offset = readManualOffset();

    // The ONLY changed composition line: pick agent value iff util_apply.
    float final_brightness = (util_apply ? util_eval.recommended : ml_brightness) + manual_offset;
    ...
    // Remember what we actually commanded, for the agent's Smooth() term.
    agent_prev_brightness = final_brightness;

    // Pot-based sample capture stays anchored to the ML output (NOT the agent
    // output) so the learning pipeline is identical whether the agent is on or
    // off. When util_apply == false this equals final_brightness exactly.
    float ml_anchored_final = ml_brightness + manual_offset;
    if (ml_anchored_final < 0)   ml_anchored_final = 0;
    if (ml_anchored_final > 100) ml_anchored_final = 100;
    monitorPotForSampleCapture(current_time, ml_brightness, ml_anchored_final);
...
void testManualPrediction(int hour, int minute, int day, float ambient,
  ...
  // ADDITIVE: show what the utility agent would recommend for this scenario.
  bool night = (getTimePeriod(hour) == 4);
  UtilityResult ua = utilityEvaluate(knn_pred, agent_prev_brightness,
                                     night, motion != 0, util_cfg);
  Serial.println();
  Serial.println(F("Utility agent (advisory view):"));
  Serial.print(F("  Recommended b*    : ")); Serial.print(ua.recommended, 0);
  Serial.print(F("%   (anchor ML="));        Serial.print(ua.ml_anchor, 1);
  Serial.println(F("%)"));
  Serial.print(F("  Night+motion floor: "));
  if (ua.floor_active) { Serial.print(F("ACTIVE >= ")); Serial.print(ua.floor_value, 0); Serial.println(F("%")); }
  else                 { Serial.println(F("inactive")); }
...
// ADDITIVE: live utility-agent breakdown using current sensors/clock.
void cmdUtility() {
  DateTime now = rtc.now();
  int hour    = now.hour();
  int minute  = now.minute();
  int day_adj = (now.dayOfTheWeek() + 6) % 7;
  int motion  = digitalRead(PIR_PIN);

  // READ-ONLY use of the ML pipeline (all out-params NULL).
  float ml = predict_knn(smoothed_ldr, motion, hour, minute, day_adj,
                         NULL, NULL, NULL, NULL, NULL, NULL);

  bool night = (getTimePeriod(hour) == 4);
  UtilityResult r = utilityEvaluate(ml, agent_prev_brightness,
                                    night, motion != 0, util_cfg);
  ...
  Serial.println();
  Serial.println(F("=== UTILITY AGENT ==="));
  Serial.print(F("Mode            : "));
  Serial.println(util_apply ? F("ENABLED (driving LED)") : F("ADVISORY only (LED uses ML)"));

  Serial.println(F("-- Inputs --"));
  Serial.print(F("  Time          : ")); Serial.print(hour); Serial.print(F(":"));
  if (minute < 10) Serial.print('0'); Serial.print(minute);
  Serial.print(F(" (")); Serial.print(period_names[getTimePeriod(hour)]); Serial.println(F(")"));
  Serial.print(F("  Day           : ")); Serial.println(day_names[day_adj]);
  Serial.print(F("  Ambient       : ")); Serial.print(smoothed_ldr, 1); Serial.println(F(" lux"));
  Serial.print(F("  Motion        : ")); Serial.println(motion ? F("YES") : F("NO"));
  Serial.print(F("  Night         : ")); Serial.println(night ? F("YES") : F("NO"));
  Serial.print(F("  ML anchor     : ")); Serial.print(r.ml_anchor, 1); Serial.println(F("%"));
  Serial.print(F("  Prev brightness: ")); Serial.print(r.prev_brightness, 1); Serial.println(F("%"));
  ...
  Serial.print(F("  min_safe_night: ")); Serial.print(util_cfg.min_safe_night, 0); Serial.println(F("%"));

  Serial.println(F("-- Safety constraint --"));
  Serial.print(F("  Night+motion floor: "));
  if (r.floor_active) { Serial.print(F("ACTIVE  b >= ")); Serial.print(r.floor_value, 0); Serial.println(F("%")); }
  else                { Serial.println(F("inactive")); }

  Serial.println(F("-- Decision (argmax over 0..100 step 5) --"));
  Serial.print(F("  Recommended b*: ")); Serial.print(r.recommended, 0); Serial.println(F("%"));
  Serial.print(F("  vs ML anchor  : ")); Serial.print(r.recommended - r.ml_anchor, 1); Serial.println(F("% delta"));

  Serial.println(F("-- U(b*) contributions --"));
  Serial.print(F("  +comfort      : ")); Serial.print(r.comfort_contrib, 4);
  Serial.print(F("  (Comfort="));        Serial.print(r.comfort, 4); Serial.println(F(")"));
  Serial.print(F("  -energy       : ")); Serial.print(r.energy_contrib, 4);
  Serial.print(F("  (Energy="));         Serial.print(r.energy, 4); Serial.println(F(")"));
  Serial.print(F("  -smooth       : ")); Serial.print(r.smooth_contrib, 4);
  Serial.print(F("  (Smooth="));         Serial.print(r.smooth, 4); Serial.println(F(")"));
  Serial.print(F("  U(b*) total   : ")); Serial.println(r.utility, 4);
  Serial.println();
}
...
void cmdModelInfo() {
  ...
  Serial.print  (F("Utility agent  : "));
  Serial.println(util_apply ? F("ENABLED (driving LED)") : F("ADVISORY only"));
  Serial.println();
}
...
void cmdStats() {
  ...
  Serial.print(F("Utility agent     : "));
  Serial.println(util_apply ? F("ENABLED") : F("ADVISORY"));
  Serial.println();
}
...
void cmdHelp() {
  ...
  Serial.println(F(""));
  Serial.println(F("=== UTILITY AGENT ==="));
  Serial.println(F("utility         - print agent breakdown for current inputs"));
  Serial.println(F("utilon          - enable agent (it drives the LED)"));
  Serial.println(F("utiloff         - advisory only (LED uses ML output) [default]"));
  Serial.println(F("setutil C E M [F] - weights comfort/energy/smooth (>=0)"));
  Serial.println(F("                    + optional night-motion floor% F (0..100)"));
  Serial.println(F(""));
...
void handleSerialCommand() {
  ...
  if (command == "help") {
    ...
    util_apply = true;
    Serial.println(F("[v] Utility agent ENABLED - now driving brightness"));
  }
  else if (command == "utiloff") {
    util_apply = false;
    Serial.println(F("[v] Utility agent ADVISORY only - LED uses ML output"));
  }
    ...
    String tok[6];
    int n = tokenizeArgs(args, tok, 6);

    if (n != 3 && n != 4) {
      Serial.print(F("[x] Expected 3 or 4 numeric arguments after 'setutil', got "));
      Serial.println(n);
      Serial.println(F("    setutil C E M       (comfort energy smooth)"));
    ...
    float f = util_cfg.min_safe_night;   // keep current floor unless provided
    if (n == 4) {
      f = tok[3].toFloat();
      if (f < 0 || f > 100) {
        Serial.print(F("[x] Floor must be 0..100, got "));
        Serial.println(f);
        return;
      }
    }
    ...
    Serial.print(F(" floor="));        Serial.print(f, 0); Serial.println(F("%"));
  }
